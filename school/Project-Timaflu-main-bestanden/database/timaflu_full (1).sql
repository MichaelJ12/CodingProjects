CREATE DATABASE  IF NOT EXISTS `timaflu` /*!40100 DEFAULT CHARACTER SET utf8mb3 */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `timaflu`;
...
INSERT INTO `adres` VALUES (1,'Kruisstraat','12','1011AB','Amsterdam'),(2,'Lindelaan','45','3512CD','Utrecht'),(3,'Dorpsstraat','78','2000EF','Rotterdam'),(4,'Beukenweg','23','3012GH','Den Haag'),(5,'Stationsplein','5','1012IJ','Amsterdam'),(6,'Hofstraat','11','3013KL','Den Haag'),(7,'Noordlaan','50','2001MN','Rotterdam'),(8,'Zuidstraat','33','3513OP','Utrecht'),(9,'Oostsingel','22','1013QR','Amsterdam'),(10,'Westend','44','3514ST','Utrecht'),(11,'Molenstraat','14','1014UV','Amsterdam'),(12,'Duinstraat','7','3515WX','Utrecht'),(13,'Bergzichtlaan','3','2002YZ','Rotterdam'),(14,'Bloemstraat','18','3015AB','Den Haag'),(15,'Rivierweg','22','1015CD','Amsterdam'),(16,'Valkenstraat','27','3516EF','Utrecht'),(17,'Lijsterlaan','9','2003GH','Rotterdam'),(18,'Eikenweg','12','3016IJ','Den Haag'),(19,'Zandpad','5','1016KL','Amsterdam'),(20,'Kastanjelaan','31','3517MN','Utrecht'),(21,'Apotheekstraat','12','1234AB','Utrecht'),(22,'Farmaciastraat','45','5678CD','Amersfoort');
...
CREATE TABLE `betaling` (
  ...
  CONSTRAINT `fk_Betaling_Factuur1` FOREIGN KEY (`factuur_id`) REFERENCES `factuur` (`factuur_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
INSERT INTO `betaling` VALUES (2,'2026-01-12',500.00,2),(3,'2026-01-14',980.00,3);
...
CREATE TABLE `factuur` (
  ...
  CONSTRAINT `fk_Factuur_Status1` FOREIGN KEY (`factuur_status`) REFERENCES `factuur_status` (`status`) ON DELETE CASCADE ON UPDATE CASCADE
...
INSERT INTO `factuur_status` VALUES ('1e herinnering','Factuur 1e herinnering'),('2e herinnering','Factuur 2e herinnering'),('betaald','Factuur is betaald'),('incasso','Factuur wordt geïncasseerd'),('open','Factuur staat open');
...
INSERT INTO `klant` VALUES (1,'Apotheek Centraal',1,1,1,5.00,'actief',12000.00),(2,'Apotheek Zuid',2,2,2,10.00,'actief',18000.00),(3,'Apotheek Noord',3,3,3,5.00,'geblokkeerd',9000.00),(4,'Apotheek West',4,4,4,0.00,'potentieel',0.00),(5,'Apotheek Oost',5,5,5,5.00,'actief',15000.00),(6,'Apotheek Berg',6,6,6,10.00,'actief',22000.00),(7,'Apotheek Bos',NULL,7,7,5.00,'geblokkeerd',8000.00),(8,'Apotheek Valk',NULL,8,8,15.00,'actief',25000.00),(9,'Apotheek Lijster',NULL,9,9,0.00,'potentieel',0.00),(10,'Apotheek Eiken',NULL,10,10,5.00,'actief',12000.00),(11,'Apotheek Molen',NULL,11,11,5.00,'actief',13000.00),(12,'Apotheek Duin',NULL,12,12,10.00,'actief',21000.00),(13,'Apotheek Bergzicht',NULL,13,13,0.00,'potentieel',0.00),(14,'Apotheek Bloem',NULL,14,14,5.00,'actief',17000.00),(15,'Apotheek Rivier',NULL,15,15,15.00,'geblokkeerd',8000.00),(16,'Apotheek Valken',NULL,16,16,10.00,'actief',22000.00),(17,'Apotheek Lijverster',NULL,17,17,0.00,'potentieel',0.00),(18,'Apotheek Eikelen',NULL,18,18,5.00,'actief',12000.00),(19,'Apotheek Zand',NULL,19,19,5.00,'actief',14000.00),(20,'Apotheek Kastanje',NULL,20,20,10.00,'actief',20000.00),(21,'Apotheek De Zon',10,21,21,NULL,'potentieel',NULL),(23,'Apotheek Vitaal',11,22,22,NULL,'actief',18500.00);
...
CREATE TABLE `levering` (
  ...
  PRIMARY KEY (`medewerker_id`,`order_id`,`levering_id`),
  ...
  CONSTRAINT `fk_Levering_Order1` FOREIGN KEY (`order_id`) REFERENCES `order` (`order_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
INSERT INTO `levering` VALUES (1,1,4,'2026-01-12','terug naar magazijn'),(2,2,2,'2026-01-10','afgeleverd'),(3,2,5,'2026-01-13','onderweg'),(4,3,3,'2026-01-11','wordt verzameld');
...
CREATE TABLE `order` (
  ...
  CONSTRAINT `fk_Order_Factuur1` FOREIGN KEY (`factuur_id`) REFERENCES `factuur` (`factuur_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `orderregel` (
  ...
  CONSTRAINT `fk_orderregel_bestelling1` FOREIGN KEY (`order_id`) REFERENCES `order` (`order_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
INSERT INTO `orderregel` VALUES (2,3,20,5.00),(3,4,60,4.00),(4,5,30,3.50),(5,6,40,2.75),(5,7,50,4.50),(7,13,40,3.75),(7,14,100,2.00),(8,15,60,5.50);
...
CREATE TABLE `picktask` (
  ...
  `picktask_status` varchar(45) NOT NULL,
  ...
  CONSTRAINT `fk_picktask_bestelling1` FOREIGN KEY (`order_id`) REFERENCES `order` (`order_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
INSERT INTO `picktask` VALUES (2,2,2,2,2,'B','klaar'),(3,3,3,3,3,'C','wordt verzameld'),(4,4,4,1,4,'D','wordt verzameld'),(5,5,5,2,1,'A','klaar');
...
CREATE TABLE `product` (
  ...
  `voorraad_status` varchar(45) NOT NULL,
  `robot_id` int NOT NULL,
  `gang` varchar(1) NOT NULL,
...
INSERT INTO `product` VALUES (1,'Paracetamol 500mg','Pijnstiller',2.50,50,200,'doos','op voorraad',1,'A'),(2,'Ibuprofen 200mg','Ontstekingsremmer',3.00,40,100,'doos','op voorraad',2,'B'),(3,'Amoxicilline 250mg','Antibioticum',5.00,30,50,'strip','op voorraad',3,'C'),(4,'Cetirizine 10mg','Anti-allergie',4.00,30,0,'doos','niet op voorraad',5,'E'),(5,'Omeprazol 20mg','Maagbescherming',3.50,25,80,'strip','op voorraad',2,'B'),(6,'Simvastatine 10mg','Cholesterol',2.75,20,60,'doos','op voorraad',3,'C'),(7,'Paracetamol 1000mg','Pijnstiller',4.50,50,15,'doos','onder minimum',1,'A'),(8,'Ibuprofen 400mg','Ontstekingsremmer',5.50,40,32,'doos','onder minimum',2,'B'),(9,'Amoxicilline 500mg','Antibioticum',7.00,30,70,'strip','op voorraad',6,'F'),(10,'Lorazepam 1mg','Slaapmiddel',6.50,20,40,'strip','op voorraad',8,'h'),(11,'Cefalexine 500mg','Antibioticum',6.00,20,10,'strip','onder minimum',1,'A'),(12,'Metformine 500mg','Diabetes',4.50,30,13,'doos','onder minimum',2,'B'),(13,'Salbutamol 100mcg','Astma',3.75,25,2,'inhaler','onder minimum',3,'C'),(14,'Aspirine 100mg','Pijnstiller',2.00,50,200,'doos','',1,'A'),(15,'Prednison 5mg','Ontstekingsremmer',5.50,20,60,'strip','',2,'B'),(16,'Furosemide 40mg','Diureticum',3.00,15,90,'doos','',3,'C'),(17,'Diazepam 5mg','Slaapmiddel',4.00,20,50,'strip','',4,'D'),(18,'Levothyroxine 50mcg','Hormoon',4.75,25,100,'doos','',1,'A'),(19,'Amoxicilline 125mg','Antibioticum',2.50,30,70,'strip','',2,'B'),(20,'Omeprazol 40mg','Maagbescherming',5.00,25,90,'strip','',8,'H'),(21,'Loratadine 10mg','Anti-allergie',4.50,30,120,'doos','',7,'G'),(22,'Simvastatine 20mg','Cholesterol',3.25,20,18,'doos','',6,'F'),(23,'Ibuprofen 600mg','Ontstekingsremmer',5.75,40,25,'doos','',5,'E'),(24,'Paracetamol 750mg','Pijnstiller',3.50,60,300,'doos','',7,'G'),(25,'Clarithromycine 500mg','Antibioticum',7.50,30,9,'strip','',4,'D');
...
INSERT INTO `verzamelbak` VALUES (1,'VB001','50kg','Vol'),(2,'VB002','40kg','Leeg'),(3,'VB003','60kg','in gebruik'),(4,'VB004','55kg','bij distributie'),(5,'VB005','45kg','leeg');
...
-- Dump completed on 2026-01-25 23:45:42
